...
// TODO:
// Add a function for reading "Memory Map select" / "Alphanumeric Mode Disable" / "Chain O/E" from 3CEh (Index 06h)
...
void print_string_to_VGA_display_buffer(u16 position, unsigned char* string, int string_size)
{
    ...
    {
        print_char_to_VGA_display_buffer(position + i, string[i], DEFAULT_BACKGROUND_COLOR, DEFAULT_FOREGROUND_COLOR);
    }
}
